...
ابتدا، بیایید تمرین را ترجمه کنیم:

**تمرین 9.4.5: ماتریس M مثال در حال اجرا را با روش‌های زیر نرمال‌سازی کنید:**
**(الف) ابتدا هر عنصر را از میانگین سطر مربوطه کم کنید، و سپس هر عنصر را از میانگین ستون (تغییریافته) مربوطه کم کنید.**
**(ب) ابتدا هر عنصر را از میانگین ستون مربوطه کم کنید، و سپس هر عنصر را از میانگین سطر (تغییریافته) مربوطه کم کنید.**
**آیا تفاوتی در نتایج (الف) و (ب) وجود دارد؟**

این تمرین از بخش 9.4 منابع شما، درباره کاهش ابعاد (Dimensionality Reduction) است. ماتریس M مثال در حال اجرا که به آن ارجاع داده شده، همان ماتریس نمایش داده شده در شکل 9.9 است.

ماتریس M اصلی به شرح زیر است:
```
5 2 4 4 3
3 3 1 2 4
1 2   3 1
...
5 4 4 5 4
```
**توجه:** در ماتریس بالا، عنصر در سطر سوم، ستون سوم **خالی** است. هنگام محاسبه میانگین‌ها، این خانه‌ی خالی نادیده گرفته می‌شود، همانطور که در بخش 9.4.5 ذکر شده است: "ما می‌توانیم از هر عنصر غیرخالی mij میانگین امتیاز کاربر i را کم کنیم."

---

**حل قسمت (الف): ابتدا نرمال‌سازی سطر، سپس ستون**

1.  **محاسبه میانگین هر سطر در ماتریس M اصلی:**
    *   سطر 1: (5 + 2 + 4 + 4 + 3) / 5 = 18 / 5 = **3.6**
    *   سطر 2: (3 + 3 + 1 + 2 + 4) / 5 = 13 / 5 = **2.6**
    *   سطر 3: (1 + 2 + 3 + 1) / 4 = 7 / 4 = **1.75** (عنصر خالی نادیده گرفته شد)
    *   سطر 4: (4 + 2 + 5 + 4 + 3) / 5 = 18 / 5 = **3.6**
    *   سطر 5: (5 + 4 + 4 + 5 + 4) / 5 = 22 / 5 = **4.4**

2.  **کم کردن میانگین هر سطر از عناصر مربوطه در آن سطر:**
    ماتریس پس از نرمال‌سازی سطر (M_row_norm) به دست می‌آید:
    ```
    [1.4, -1.6, 0.4, 0.4, -0.6]  (5-3.6, 2-3.6, ..., 3-3.6)
    [0.4, 0.4, -1.6, -0.6, 1.4]  (3-2.6, 3-2.6, ..., 4-2.6)
    [-0.75, 0.25, nan, 1.25, -0.75] (1-1.75, 2-1.75, nan, 3-1.75, 1-1.75)
    [0.4, -1.6, 1.4, 0.4, -0.6]  (4-3.6, 2-3.6, ..., 3-3.6)
    [0.6, -0.4, -0.4, 0.6, -0.4]  (5-4.4, 4-4.4, ..., 4-4.4)
    ```

3.  **محاسبه میانگین هر ستون در ماتریس M_row_norm (فقط عناصر غیرخالی):**
    *   ستون 1: (1.4 + 0.4 - 0.75 + 0.4 + 0.6) / 5 = 2.05 / 5 = **0.41**
    *   ستون 2: (-1.6 + 0.4 + 0.25 - 1.6 - 0.4) / 5 = -2.95 / 5 = **-0.59**
    *   ستون 3: (0.4 - 1.6 + 1.4 - 0.4) / 4 = -0.2 / 4 = **-0.05** (عنصر خالی نادیده گرفته شد)
    *   ستون 4: (0.4 - 0.6 + 1.25 + 0.4 + 0.6) / 5 = 2.05 / 5 = **0.41**
    *   ستون 5: (-0.6 + 1.4 - 0.75 - 0.6 - 0.4) / 5 = -0.95 / 5 = **-0.19**

4.  **کم کردن میانگین هر ستون از عناصر مربوطه در آن ستون:**
    ماتریس نهایی برای قسمت (الف) (M_final_a) به دست می‌آید:
    ```
    [0.99, -1.01, 0.45, -0.01, -0.41]
    [-0.01, 0.99, -1.55, -1.01, 1.59]
    [-1.16, 0.84, nan, 0.84, -0.56]
    [-0.01, -1.01, 1.45, -0.01, -0.41]
    [0.19, 0.19, -0.35, 0.19, -0.21]
    ```

---

**حل قسمت (ب): ابتدا نرمال‌سازی ستون، سپس سطر**

1.  **محاسبه میانگین هر ستون در ماتریس M اصلی:**
    *   ستون 1: (5 + 3 + 1 + 4 + 5) / 5 = 18 / 5 = **3.6**
    *   ستون 2: (2 + 3 + 2 + 2 + 4) / 5 = 13 / 5 = **2.6**
    *   ستون 3: (4 + 1 + 5 + 4) / 4 = 14 / 4 = **3.5** (عنصر خالی نادیده گرفته شد)
    *   ستون 4: (4 + 2 + 3 + 4 + 5) / 5 = 18 / 5 = **3.6**
    *   ستون 5: (3 + 4 + 1 + 3 + 4) / 5 = 15 / 5 = **3.0**

2.  **کم کردن میانگین هر ستون از عناصر مربوطه در آن ستون:**
    ماتریس پس از نرمال‌سازی ستون (M_col_norm) به دست می‌آید:
    ```
    [1.4, -0.6, 0.5, 0.4, 0.0]   (5-3.6, 2-2.6, ..., 3-3.0)
    [-0.6, 0.4, -2.5, -1.6, 1.0]  (3-3.6, 3-2.6, ..., 4-3.0)
    [-2.6, -0.6, nan, -0.6, -2.0] (1-3.6, 2-2.6, nan, 3-3.6, 1-3.0)
    [0.4, -0.6, 1.5, 0.4, 0.0]   (4-3.6, 2-2.6, ..., 3-3.0)
    [1.4, 1.4, 0.5, 1.4, 1.0]   (5-3.6, 4-2.6, ..., 4-3.0)
    ```

3.  **محاسبه میانگین هر سطر در ماتریس M_col_norm (فقط عناصر غیرخالی):**
    *   سطر 1: (1.4 - 0.6 + 0.5 + 0.4 + 0.0) / 5 = 1.7 / 5 = **0.34**
    *   سطر 2: (-0.6 + 0.4 - 2.5 - 1.6 + 1.0) / 5 = -3.3 / 5 = **-0.66**
    *   سطر 3: (-2.6 - 0.6 - 0.6 - 2.0) / 4 = -5.8 / 4 = **-1.45** (عنصر خالی نادیده گرفته شد)
    *   سطر 4: (0.4 - 0.6 + 1.5 + 0.4 + 0.0) / 5 = 1.7 / 5 = **0.34**
    *   سطر 5: (1.4 + 1.4 + 0.5 + 1.4 + 1.0) / 5 = 5.7 / 5 = **1.14**

4.  **کم کردن میانگین هر سطر از عناصر مربوطه در آن سطر:**
    ماتریس نهایی برای قسمت (ب) (M_final_b) به دست می‌آید:
    ```
    [1.06, -0.94, 0.16, 0.06, -0.34]
    [0.06, 1.06, -1.84, -0.94, 1.66]
    [-1.15, 0.85, nan, 0.85, -0.55]
    [0.06, -0.94, 1.16, 0.06, -0.34]
    [0.26, 0.26, -0.64, 0.26, -0.14]
    ```

---

**مقایسه نتایج (الف) و (ب): آیا تفاوتی در نتایج وجود دارد؟**

با مقایسه ماتریس نهایی M_final_a و M_final_b، به وضوح مشاهده می‌شود که **نتایج متفاوت هستند**. مقادیر در موقعیت‌های متناظر در دو ماتریس یکسان نیستند.

این نتیجه با توضیحات موجود در منبع هم‌خوانی دارد، که بیان می‌کند: "نتایجی که از انجام کارها به این دو ترتیب متفاوت به دست می‌آید لزوماً یکسان نیستند، اما تمایل دارند نزدیک به هم باشند."

**نتیجه‌گیری:**
بله، **تفاوت‌هایی در نتایج نرمال‌سازی ماتریس M بسته به ترتیب اعمال عملیات (ابتدا سطرها و سپس ستون‌ها، یا برعکس) وجود دارد.** این نشان می‌دهد که ترتیب نرمال‌سازی بر مقادیر نهایی ماتریس تأثیر می‌گذارد، حتی اگر نتایج "نزدیک" باشند.
