...
در ادامه به ترجمه و پاسخ تمرینات بخش 9.4 از منبع "bookL.pdf" پرداخته شده است:

---

**تمرین 9.4.1**
**ترجمه:**
"با شروع از تجزیه ماتریس در شکل 9.10، می‌توانیم هر یک از 20 ورودی در U یا V را برای بهینه‌سازی انتخاب کنیم. این مرحله بهینه‌سازی اولیه را با فرض انتخاب:
(الف) **u32**
(ب) **v41**
انجام دهید."
...
**نکته مهم:** قبل از شروع حل، لازم به ذکر است که در بخش (ب) این تمرین، **v41** به عنوان ورودی برای بهینه‌سازی خواسته شده است. با توجه به اینکه ماتریس **V** (شکل 9.9 و 9.10) دارای ابعاد 2x5 است، ورودی **v41** (سطر 4، ستون 1) خارج از محدوده این ماتریس قرار می‌گیرد. این یک ابهام در صورت سوال است. بنابراین، **پاسخ تنها به بخش (الف) تمرین داده می‌شود** و برای بخش (ب)، نیاز به شفاف‌سازی از سوی شما وجود دارد.

**حل بخش (الف) u32:**

هدف ما بهینه‌سازی ورودی **u32** (عنصر سطر 3 و ستون 2 ماتریس U) است. مقدار اولیه این عنصر از شکل 9.10 برابر با 1 است.

**مراحل حل:**
1.  **شناسایی مقادیر اولیه:**
    *   ماتریس **M** از شکل 9.9:
        ```
        M = [,
            ,
            ...
            ,
             [5, 4, 4, 5, BLANK]]
        ```
    *   ماتریس‌های اولیه **U** و **V** از شکل 9.10 (همه ورودی‌ها 1 هستند):
        ```
        U = [,
            ,
            ,  <-- Row 3
        ...
        ```
    *   شاخص‌های مورد نظر برای **u32** عبارتند از: **r = 3** (سطر) و **s = 2** (ستون).
    *   بعد مشترک **d = 2** است.

2.  **فرمول بهینه‌سازی:**
    برای بهینه‌سازی عنصر **u_rs** (که در اینجا **u32** است)، از فرمول زیر استفاده می‌کنیم:
    `x = (∑_j v_sj * (m_rj - ∑_(k≠s) u_rk * v_kj)) / (∑_j v_sj^2)`
    *   **x** مقدار بهینه جدید برای **u_rs** است.
    *   **∑_j** به معنای جمع روی تمام ستون‌های **j** است که **m_rj** (عنصر متناظر در ماتریس M) **خالی (BLANK) نیستند**.
    *   **k≠s** به معنای جمع روی تمام **k**ها از 1 تا **d** است، به جز **k = s**.

3.  **محاسبه مخرج کسر:**
    مخرج کسر عبارت است از: `∑_j v_sj^2`
    *   در اینجا **s = 2** است، بنابراین ما به مقادیر سطر دوم ماتریس **V** نیاز داریم: **v21, v22, v23, v24, v25**.
    *   با توجه به ماتریس اولیه **V**، تمام این مقادیر **1** هستند.
    *   عناصر غیرخالی سطر **r = 3** ماتریس **M** عبارتند از: **m31, m32, m34, m35** (m33 خالی است).
    *   بنابراین، **j**ها که در جمع شرکت می‌کنند عبارتند از **1, 2, 4, 5**.
    *   **مخرج = v21^2 + v22^2 + v24^2 + v25^2 = 1^2 + 1^2 + 1^2 + 1^2 = 1 + 1 + 1 + 1 = 4**

4.  **محاسبه صورت کسر:**
    صورت کسر عبارت است از: `∑_j v_sj * (m_rj - ∑_(k≠s) u_rk * v_kj)`
    *   در اینجا **r = 3** و **s = 2**.
    *   **k≠s** به معنای **k≠2** است، بنابراین **k = 1** (زیرا **d = 2**).
    *   عبارت `∑_(k≠s) u_rk * v_kj` ساده می‌شود به `u_r1 * v_1j` یعنی `u_31 * v_1j`.
    *   از ماتریس اولیه **U**، **u_31 = 1**.
    *   از ماتریس اولیه **V**، تمام **v_1j**ها **1** هستند.
    *   بنابراین، `u_31 * v_1j = 1 * 1 = 1` برای تمام **j**ها.

    حالا عبارت `m_rj - (u_31 * v_1j)` را برای هر **j** غیرخالی در سطر 3 ماتریس **M** محاسبه می‌کنیم:
    *   برای **j = 1 (m31 = 1)**: `(m31 - u_31*v_11) = (1 - 1*1) = 0`
    *   برای **j = 2 (m32 = 2)**: `(m32 - u_31*v_12) = (2 - 1*1) = 1`
    *   برای **j = 4 (m34 = 3)**: `(m34 - u_31*v_14) = (3 - 1*1) = 2`
    *   برای **j = 5 (m35 = 1)**: `(m35 - u_31*v_15) = (1 - 1*1) = 0`

    سپس، این نتایج را در `v_sj` (یعنی `v_2j` که همه 1 هستند) ضرب کرده و با هم جمع می‌کنیم:
    *   **صورت = (v21 * 0) + (v22 * 1) + (v24 * 2) + (v25 * 0)**
    *   **صورت = (1 * 0) + (1 * 1) + (1 * 2) + (1 * 0) = 0 + 1 + 2 + 0 = 3**

5.  **محاسبه مقدار بهینه x:**
    **x = صورت / مخرج = 3 / 4 = 0.75**

**نتیجه:**
مقدار بهینه جدید برای **u32** برابر با **0.75** است. پس از این مرحله، ماتریس U به صورت زیر خواهد بود:
```
U = [,
    ,
     [1, 0.75],  <-- Updated u32
...
**تمرین 9.4.2**
**ترجمه:**
"اگر بخواهیم مانند شکل 9.10، همه ورودی‌های U و V را با یک مقدار ثابت شروع کنیم، چه مقداری RMSE را برای ماتریس M مثال ما به حداقل می‌رساند؟"

**پاسخ:**

**مراحل حل:**
1.  **تعریف مقادیر اولیه و محصول UV:**
    فرض می‌کنیم که همه ورودی‌های ماتریس **U** و **V** دارای یک مقدار ثابت **z** هستند.
    *   **U** یک ماتریس 5x2 است که همه `u_ik = z`.
    *   **V** یک ماتریس 2x5 است که همه `v_kj = z`.
    *   محصول **P = UV** یک ماتریس 5x5 خواهد بود. عنصر **P_ij** از این محصول به صورت زیر محاسبه می‌شود:
        `P_ij = ∑_k (u_ik * v_kj)`
    *   با توجه به اینکه بعد مشترک **d = 2** است، جمع روی **k=1, 2** انجام می‌شود.
    *   `P_ij = (u_i1 * v_1j) + (u_i2 * v_2j) = (z * z) + (z * z) = z^2 + z^2 = 2z^2`
    *   بنابراین، تمام عناصر ماتریس **P** برابر با **2z^2** خواهند بود.

2.  **فرمول RMSE (خطای مجذور میانگین ریشه):**
    ...
    `Sum_Squared_Error = ∑_i ∑_j (m_ij - P_ij)^2` (برای تمام `m_ij`های غیرخالی)
    `Sum_Squared_Error = ∑_i ∑_j (m_ij - 2z^2)^2` (برای تمام `m_ij`های غیرخالی)

3.  **یافتن مقدار ثابت بهینه:**
...
4.  **محاسبه میانگین عناصر غیرخالی ماتریس M:**
    لیست عناصر غیرخالی ماتریس **M** از شکل 9.9:
    *   سطر 1: 5, 2, 4, 4, 3 (مجموع = 18)
    *   سطر 2: 3, 3, 1, 2, 4 (مجموع = 13)
    *   سطر 3: 1, 2, 3, 1 (مجموع = 7 - عنصر سطر 3، ستون 3 خالی است)
    *   سطر 4: 4, 2, 5, 4, 3 (مجموع = 18)
    *   سطر 5: 5, 4, 4, 5 (مجموع = 18 - عنصر سطر 5، ستون 5 خالی است)

    *   **تعداد کل عناصر غیرخالی = 5 + 5 + 4 + 5 + 4 = 23**
    *   **مجموع کل عناصر غیرخالی = 18 + 13 + 7 + 18 + 18 = 74**

    *   **میانگین = 74 / 23 ≈ 3.21739**
...
**نتیجه:**
مقدار **z ≈ 1.26834** برای همه ورودی‌های **U** و **V**، RMSE را برای ماتریس **M** به حداقل می‌رساند.
